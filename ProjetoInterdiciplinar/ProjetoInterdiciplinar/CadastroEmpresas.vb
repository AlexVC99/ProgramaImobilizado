Public Class CadastroEmpresas
    ...
    Private Sub txt_busca_TextChanged(sender As Object, e As EventArgs) Handles txt_busca.TextChanged
        sql = "select * from Empresas where Cadastro like '" & txt_busca.Text & "%' and usuario='" & user & "' or Nome like '" & txt_busca.Text & "%'and usuario='" & user & "'"
        rs = db.Execute(sql)
        cont = 1
    ...
    Private Sub dgv_empresas_CellContentClick(sender As Object, e As DataGridViewCellEventArgs) Handles dgv_empresas.CellContentClick
        With dgv_empresas
            ...
            ElseIf .CurrentRow.Cells(5).Selected Then
                resp = MsgBox("Deseja Remover a empresa selecionada ira apagar tambem todas suas relações e bens ?", MsgBoxStyle.Question + MsgBoxStyle.YesNo, "ATENÇÃO")
                If resp = MsgBoxResult.Yes Then
                    aux = .CurrentRow.Cells(0).Value
                    sql = "Delete from Empresas where Cadastro=" & aux & ""
                    rs = db.Execute(sql)
                    sql = "Delete from Bens where Codigo=" & aux & ""
                    rs = db.Execute(sql)
                    sql = "Delete from Vinculadas where CodEmpresa=" & aux & ""
                    rs = db.Execute(sql)
